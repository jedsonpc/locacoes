...
const APP_VERSION_LABEL = "v2.1.48-auto-20260803-1514";
...
const moneyFieldKeys = new Set(["baseDaily", "cleaningFee", "defaultSecurityDeposit", "reservationTotal", "dailyRate", "discount", "deposit", "secondPayment", "finalPayment", "amount"]);
...
function normalizeContract(contract = {}) {
  ...
  return {
    ...
    deposit: received,
    secondPayment: toNumber(contract.secondPayment),
    finalPayment: toNumber(contract.finalPayment),
    secondPaymentPaid: contract.secondPaymentPaid === true || contract.secondPaymentPaid === "sim" ? "sim" : "nao",
...
function contractTotals(contract) {
  ...
  const received = contract.paymentStatus === "pago" ? total : toNumber(contract.deposit);
...
function mobileCompactEligible() {
  return window.innerWidth <= 680 || (window.innerWidth <= 1100 && window.innerHeight <= 600) || Math.min(screen.width, screen.height) <= 600;
}
...
function dashboard() {
  ...
  const installmentAlerts = state.contracts
    .filter((contract) => contract.status !== "cancelada" && contract.paymentStatus === "parcial")
    .flatMap((contract) => {
      const apartment = getById("apartments", contract.apartmentId)?.name || "Apartamento";
      const client = getById("clients", contract.clientId)?.name || "Hóspede não informado";
      return [
        { label: "2ª parcela", amount: contract.secondPayment, dueDate: contract.secondPaymentDueDate, paid: contract.secondPaymentPaid },
        { label: "Parcela final", amount: contract.finalPayment, dueDate: contract.finalPaymentDueDate, paid: contract.finalPaymentPaid }
      ].filter((installment) => toNumber(installment.amount) > 0 && installment.paid !== "sim" && (installment.dueDate === today || installment.dueDate === tomorrow))
        .map((installment) => ({
          title: `${installment.dueDate === today ? "Vence hoje" : "Vence amanhã"}: ${installment.label} - ${apartment}`,
          text: `${client} - ${money(installment.amount)} - vencimento ${dateBR(installment.dueDate)}`,
          cls: installment.dueDate === today ? "danger" : "warn"
        }));
    });
  const paymentAlerts = state.contracts
    .filter((contract) => {
      ...
      if (contract.paymentStatus === "parcial") return false;
...
function contractDocumentHtml(contract) {
  ...
  const paymentItems = contract.paymentStatus === "pago"
  ...
  return `<article class="contract-document" id="contractDocument">
    ...
    <p>O pagamento deve ser efetuado da seguinte forma:</p><ol>${paymentItems}</ol>
...
function reportsView() {
  ...
  const annualYear = Number(state.settings.reportAnnualYear) || currentYear;
...
function reportScopeTag(apartmentId = "") {
  const label = apartmentId ? (getById("apartments", apartmentId)?.name || "Imovel selecionado") : "Todos os imoveis";
  return `<span class="report-scope-tag"><small>Escopo do relatorio</small><strong>${escapeHtml(label)}</strong></span>`;
}
...
function reportComparisonPanel(current, previous, previousStart, previousEnd, apartmentId = "") {
  const rows = [
  ...
  return `<section class="panel"><div class="toolbar"><div><p class="eyebrow">Comparativo anual</p><h2>Periodo filtrado x mesmo periodo do ano anterior</h2>${reportScopeTag(apartmentId)}</div><span class="muted">Ano anterior: ${dateBR(previousStart)} a ${dateBR(previousEnd)}</span></div><div class="comparison-list">${rows}</div></section>`;
}
...
function reportPeriodBuckets(start, end, contracts) {
  ...
  const buckets = [];
  if (days <= 45) {
    for (let date = start; date <= end; date = addDays(date, 1)) buckets.push({ start: date, end: date, label: dateBR(date).slice(0, 5), nights: 0, revenue: 0 });
  } else if (days <= 180) {
    for (let date = start; date <= end; date = addDays(date, 7)) {
      const bucketEnd = addDays(date, 6) > end ? end : addDays(date, 6);
      buckets.push({ start: date, end: bucketEnd, label: dateBR(date).slice(0, 5), nights: 0, revenue: 0 });
    }
  } else {
    ...
    while (cursor <= finalDate) {
      ...
      buckets.push({ start: monthStart, end: monthEnd, label: `${String(cursor.getUTCMonth() + 1).padStart(2, "0")}/${cursor.getUTCFullYear()}`, nights: 0, revenue: 0 });
      cursor = new Date(Date.UTC(cursor.getUTCFullYear(), cursor.getUTCMonth() + 1, 1));
    }
  }
  contracts.forEach((contract) => {
    const totals = contractTotals(contract);
    const totalNights = Math.max(1, totals.stayNights);
    buckets.forEach((bucket) => {
      const begin = parseDate(contract.checkIn) > parseDate(bucket.start) ? parseDate(contract.checkIn) : parseDate(bucket.start);
      const bucketFinish = parseDate(addDays(bucket.end, 1));
      const finish = parseDate(contract.checkOut) < bucketFinish ? parseDate(contract.checkOut) : bucketFinish;
      const occupiedNights = Math.max(0, Math.round((finish - begin) / oneDay));
      if (!occupiedNights) return;
      bucket.nights += occupiedNights;
      bucket.revenue += totals.total * (occupiedNights / totalNights);
    });
  });
  buckets.forEach((bucket) => {
    bucket.nights = occupiedNightKeys(contracts, bucket.start, bucket.end).size;
  });
  return buckets;
}

function reportEvolutionPanel(buckets, apartmentId = "") {
  const nights = reportBarChart("Diarias ocupadas por imovel", buckets, "nights", (value) => String(Math.round(value)), (value) => `${Math.round(value)} diaria(s) ocupada(s)`);
  const revenue = reportBarChart("Faturamento proporcional", buckets, "revenue", money, money);
  return `<section class="panel"><div class="toolbar"><div><p class="eyebrow">Evolucao</p><h2>Diarias e faturamento</h2>${reportScopeTag(apartmentId)}</div></div><p class="muted block-help">Cada diaria ocupada e contada por imovel; ao selecionar todos, as diarias das unidades sao somadas. O check-out nao conta como diaria. O faturamento e distribuido proporcionalmente pelas diarias ocupadas.</p><div class="report-chart-grid">${nights}${revenue}</div></section>`;
}
...
function annualBrokerRevenuePanel(year, apartmentId = "") {
  ...
  if (!brokerRows.length) return `<section class="panel"><div class="toolbar"><div><p class="eyebrow">Analise anual</p><h2>Faturamento mensal por corretor - ${year}</h2>${reportScopeTag(apartmentId)}</div><div class="filters">${yearFilter}</div></div>${empty("Nenhuma locacao encontrada no ano e escopo selecionados.")}</section>`;
  ...
  const executiveKpis = `<div class="annual-executive-kpis"><article><span>Faturamento por corretores</span><strong>${money(grandTotal)}</strong><small>${totalDailyCount} diaria(s) vinculada(s)</small></article><article><span>Lider de faturamento</span><strong>${escapeHtml(topBroker.name)}</strong><small>${money(topBroker.total)} - ${percent(grandTotal ? topBroker.total / grandTotal : 0)}</small></article><article><span>Melhor competencia</span><strong>${monthLabels[bestMonthIndex]}/${year}</strong><small>${money(monthlyTotals[bestMonthIndex])}</small></article></div>`;
  ...
  if (collection === "contracts") {
    ...
    const updateCourtesyFields = () => {
      ...
      courtesyMoneyFields.forEach((id) => {
        ...
        input.disabled = isCourtesy || id === "dailyRate";
    ...
    const updateInstallmentFields = () => {
      ...
      const first = toNumber(document.querySelector("#deposit")?.value);
      const second = toNumber(document.querySelector("#secondPayment")?.value);
      const completedBySecond = total > 0 && first + second >= total;
      installmentKeys.forEach((key) => {
        const input = document.querySelector(`#${key}`);
        const field = document.querySelector(`[data-field-key="${key}"]`);
        if (!input || !field) return;
        field.hidden = !isPartial;
        const finalField = ["finalPayment", "finalPaymentDueDate", "finalPaymentPaid"].includes(key);
        const emptyInstallment = (key === "secondPaymentPaid" && second <= 0) || (key === "finalPaymentPaid" && toNumber(document.querySelector("#finalPayment")?.value) <= 0);
        input.disabled = !isPartial || (finalField && completedBySecond) || emptyInstallment;
      });
      const finalPayment = document.querySelector("#finalPayment");
      if (finalPayment && completedBySecond) finalPayment.value = brazilianValue(0);
    };
    paymentStatusSelect?.addEventListener("change", updateInstallmentFields);
    ["reservationTotal", "deposit", "secondPayment", "finalPayment"].map((id) => document.querySelector(`#${id}`)).filter(Boolean).forEach((input) => input.addEventListener("input", updateInstallmentFields));
...
function submitForm(event) {
  ...
  if (collection === "contracts") {
    ...
    if (isCourtesy) {
      ["reservationTotal", "dailyRate", "cleaningFee", "discount", "deposit", "secondPayment", "finalPayment"].forEach((key) => record[key] = 0);
      record.commissionAlreadyDeducted = "nao";
      record.paymentConfirmed = "nao";
    }
    if (record.paymentStatus !== "parcial") {
      ["deposit", "secondPayment", "finalPayment"].forEach((key) => record[key] = 0);
      ["firstPaymentDate", "secondPaymentDueDate", "finalPaymentDueDate"].forEach((key) => record[key] = "");
      record.secondPaymentPaid = "nao";
      record.finalPaymentPaid = "nao";
    } else if (toNumber(record.deposit) + toNumber(record.secondPayment) >= toNumber(record.reservationTotal)) {
      record.finalPayment = 0;
      record.finalPaymentDueDate = "";
      record.finalPaymentPaid = "nao";
    }
...
function bindViewEvents() {
  ...
  document.querySelector("#reportPeriodStart")?.addEventListener("change", (event) => {
    state.settings.reportPeriodStart = event.target.value;
...
function getAccessUrl() {
  ...
  url.searchParams.set("v", "2.1.48-auto-20260803-1514");
  return url.toString();
}
...
async function logout() {
  ...
  location.replace("login.html?v=2.1.48-auto-20260803-1514");
}
...
document.addEventListener("click", (event) => {
  ...
  if (clearPeriodReportBtn) {
    ...
    state.settings.reportPeriodEnd = "";
    saveState("report_period_clear");
    render();
    return;
  }
